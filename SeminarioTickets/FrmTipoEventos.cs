...
namespace SeminarioTickets
{
    public partial class FrmTipoEventos : Form
    {
        ...
        {
            InitializeComponent();
            dgvTipoEvn.CellClick += dgvTipoEvn_CellClick;
        }
        ...
        {
            conexion.Modificaciones(" exec InsercionTipoEventos'" + txtId.Text + "','" + txtNombre.Text + "'  ");
            ...
            conexion.Grids("SELECT IdEvn AS ID, NomEvn as TipoEventos FROM TipoEventos", dgvTipoEvn);
        }
        ...
        {
            conexion.Modificaciones("exec ModificarTipoEventos '" + txtId.Text + "', '" + txtNombre.Text + "' ");
        ...
        {
            conexion.Modificaciones("exec EliminarTipoEventos '" + txtId.Text + "' ");
            ...
            //Visualización de datos de la base al DataGridView
            conexion.Grids("SELECT IdEvn AS Id, NomEvn as TipoEvento FROM TipoEventos", dgvTipoEvn);
        }

        private void dgvTipoEvn_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0) // Asegúrate de que la selección no sea la fila de encabezado
            {
                DataGridViewRow row = dgvTipoEvn.Rows[e.RowIndex];

                // Obtiene el valor de la celda seleccionada y asígnalo al TextBox
                txtId.Text = row.Cells["Id"].Value.ToString();
                txtNombre.Text = row.Cells["TipoEvento"].Value.ToString();
